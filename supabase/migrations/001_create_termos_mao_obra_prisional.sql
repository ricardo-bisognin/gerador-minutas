...
create table if not exists public.tc_convenentes (
  id uuid primary key default gen_random_uuid(), tipo text not null check (tipo in ('empresa','municipio')),
  nome_razao_social text not null, cnpj text not null unique, endereco text, bairro text, cidade text, uf text default 'RS', cep text,
  telefone text, email text, representante_nome text, representante_cpf text, representante_rg text,
  representante_orgao_expedidor text, representante_cargo text, representante_funcao text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table if not exists public.tc_drpp (id smallint primary key, nome text not null, sede text not null, descricao text, created_at timestamptz not null default now());
create table if not exists public.tc_estabelecimentos_penais (
  id uuid primary key default gen_random_uuid(), drpp_id smallint references public.tc_drpp(id), nome text not null,
  municipio text, endereco text, latitude numeric, longitude numeric, ativo boolean not null default true,
  created_at timestamptz not null default now(), unique(drpp_id, nome)
);
create table if not exists public.tc_solicitacao_estabelecimentos (
  ...
  estabelecimento_id uuid not null references public.tc_estabelecimentos_penais(id), primary key (solicitacao_id, estabelecimento_id)
);
create table if not exists public.tc_solicitacao_atividades (
  id uuid primary key default gen_random_uuid(), solicitacao_id uuid not null references public.tc_solicitacoes(id) on delete cascade,
  descricao text not null, ordem integer not null default 1
);
create table if not exists public.tc_documentos_submetidos (
  id uuid primary key default gen_random_uuid(), solicitacao_id uuid not null references public.tc_solicitacoes(id) on delete cascade,
  tipo_documento text not null, nome_arquivo text not null, tamanho_bytes bigint, mime_type text,
